...
def detect_face(img, min_size=40, max_dim=600, padding=80, ret_conf=False):
    ...
    if ret_conf:
        return np.array(bbs), np.array(points), np.array(confs)
    else:
        data = list(zip(bbs, points))
        data.sort(key=lambda x: (x[0][2] - x[0][0]) * (x[0][3] - x[0][1]))
        data = np.array(list(reversed(data)),dtype=object)
        bbs = np.array([x[0] for x in data])
        points = np.array([x[1] for x in data])
        return np.array(bbs), np.array(points)
