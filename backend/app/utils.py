...
from app.extensions import parser, client
...
def send_error(data=None, message="Error", code=400, version=1, status=False):
    """
...
def check_email_contact(str_email):
    if re.match(r"^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$", str_email):
        return 
    else:
         return send_error(message='Email không hợp lệ')
...
def check_phone_number(str_data):
    if re.match(r"^[\d\s()+-]{7,20}$", str_data):
        return 
    else:
         return send_error(message='Số điện thoại')
...
def notification(content=None, note=None, user_id=None,type=None):
    data = {
        '_id': str(ObjectId()),
        'content': content,
        'note': note,
