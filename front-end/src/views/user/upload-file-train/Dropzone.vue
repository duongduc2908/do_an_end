...
export default {
  ...
  methods: {
    saveFile(){
      const element = document.getElementById(this.id)
      const vm = this
      let listFile = this.dropzone.getAcceptedFiles();
      // listFile[0].upload.filename = listFile[0].upload.filename+"||"+this.full_name
      // debugger
      if(listFile.length != 0){
          store.dispatch('upload_file/add_file',{listFile:listFile,MaNV:this.MaNV}).then(res=>{
          if(res.code == 200){
            this.$emit('outputSaveFile',res);
            this.removeAllFiles()
